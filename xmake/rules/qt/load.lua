    ...
    for _, linkdir in ipairs(linkdirs) do
        for _, libpath in ipairs(os.files(path.join(linkdir, libpattern))) do
            ...
            if basename:startswith("libQt" .. qt_sdkver:major() .. "Bundled") or
                ...
                if (is_mode("debug") and basename:endswith(debug_suffix)) or
                    (not is_mode("debug") and
                        not basename:endswith(debug_suffix)) then
                    table.insert(links,
                                 core_target.linkname(path.filename(libpath)))
                end
            end
        end
    end
...
function _add_plugins(target, plugins)
    for name, plugin in pairs(plugins) do
        ...
        if plugin.linkdirs then
            target:values_add("qt.linkdirs",
                              table.unpack(table.wrap(plugin.linkdirs)))
        end
...
function _get_frameworks_from_target(target)
    ...
    for _, value in ipairs((target:get_from("__qt_frameworks", "dep::*",
                                            {interface = true}))) do
        table.join2(values, value)
    end
...
function _add_qmakeprllibs(target, prlfile, qtlibdir)
    if os.isfile(prlfile) then
        ...
        if envs.QMAKE_PRL_LIBS_FOR_CMAKE then
            for _, lib in ipairs(envs.QMAKE_PRL_LIBS_FOR_CMAKE:split(';', {
                plain = true
            })) do
                if lib:startswith("-L") then
                    ...
                    local libstr = string.gsub(lib, "%$%$%[QT_INSTALL_LIBS%]",
                                               qtlibdir)
...
function main(target, opt)
    ...
    if qt.sdkver then
        ...
        raise(
            "Qt SDK version not found, please run `xmake f --qt_sdkver=xxx` to set it.")
    end
    ...
    if plugins then
        local importfile = path.join(config.buildir(), ".qt", "plugin",
                                     target:name(), "static_import.cpp")
        ...
        if framework:startswith("Qt") then
            ...
            if framework:lower():endswith("private") then
                ...
                if target:is_plat("macosx") then
                    local frameworkdir =
                        path.join(qt.libdir, framework .. ".framework")
                    if os.isdir(frameworkdir) then
                        _add_includedirs(target, path.join(frameworkdir,
                                                           "Headers", qt.sdkver))
                        _add_includedirs(target, path.join(frameworkdir,
                                                           "Headers", qt.sdkver,
                                                           private_dir))
                    else
                        _add_includedirs(target, path.join(qt.includedir,
                                                           private_dir,
                                                           qt.sdkver,
                                                           private_dir))
                        _add_includedirs(target, path.join(qt.includedir,
                ...
                -- add definitions
                target:add("defines",
                           "QT_" .. framework:sub(3):upper() .. "_LIB")

                -- add includedirs
                if target:is_plat("macosx") then
                    local frameworkdir =
                        path.join(qt.libdir, framework .. ".framework")
                    if os.isdir(frameworkdir) and
                        os.isdir(path.join(frameworkdir, "Headers")) then
                        _add_includedirs(target,
                                         path.join(frameworkdir, "Headers"))
                        ...
                        local link = _link(target, qt.libdir, framework,
                                           qt_sdkver, infix)
                        target:add("syslinks", link)
                        _add_qmakeprllibs(target,
                                          path.join(qt.libdir, link .. ".prl"),
                                          qt.libdir)
                        _add_includedirs(target,
                                         path.join(qt.includedir, framework))
    ...
    target:add("syslinks",
               _find_static_links_3rd(target, qt.libdir, qt_sdkver, libpattern))
    ...
    if target:is_plat("macosx") then
        target:add("frameworks", "DiskArbitration", "IOKit", "CoreFoundation",
                   "CoreGraphics", "OpenGL")
        target:add("frameworks", "Carbon", "Foundation", "AppKit", "Security",
                   "SystemConfiguration")
    ...
    elseif target:is_plat("wasm") then
        ...
        for _, framework in ipairs(qt_frameworks) do
            ...
            for _, filepath in ipairs(os.files(
                                          path.join(qt.libdir, "objects-*",
                                                    prefix .. "_resources_*",
                                                    ".rcc", "*.o"))) do
                table.insert(target:objectfiles(), filepath)
            end
        end
        target:add("ldflags", "-s FETCH=1", "-s ERROR_ON_UNDEFINED_SYMBOLS=1",
                   "-s ALLOW_MEMORY_GROWTH=1", "--bind")
        target:add("shflags", "-s FETCH=1", "-s ERROR_ON_UNDEFINED_SYMBOLS=1",
                   "-s ALLOW_MEMORY_GROWTH=1", "--bind")
        if qt_sdkver:ge("6.0") then
            -- @see https://github.com/xmake-io/xmake/issues/4137
            target:add("ldflags", "-s MAX_WEBGL_VERSION=2", "-s WASM_BIGINT=1",
            ...
            target:add("ldflags", "-s MODULARIZE=1",
                       "-s EXPORT_NAME=createQtAppInstance")
            target:add("shflags", "-s MAX_WEBGL_VERSION=2", "-s WASM_BIGINT=1",
                       "-s DISABLE_EXCEPTION_CATCHING=1")
            target:add("shflags",
                       "-sASYNCIFY_IMPORTS=qt_asyncify_suspend_js,qt_asyncify_resume_js")
            target:add("shflags",
                       "-s EXPORTED_RUNTIME_METHODS=UTF16ToString,stringToUTF16,JSEvents,specialHTMLTargets")
            target:add("shflags", "-s MODULARIZE=1",
                       "-s EXPORT_NAME=createQtAppInstance")
            target:set("extension", ".js")
        else
            target:add("ldflags", "-s WASM=1", "-s FULL_ES2=1", "-s FULL_ES3=1",
                       "-s USE_WEBGL2=1")
            target:add("ldflags",
                       "-s EXPORTED_RUNTIME_METHODS=[\"UTF16ToString\",\"stringToUTF16\"]")
            target:add("shflags", "-s WASM=1", "-s FULL_ES2=1", "-s FULL_ES3=1",
                       "-s USE_WEBGL2=1")
            target:add("shflags",
                       "-s EXPORTED_RUNTIME_METHODS=[\"UTF16ToString\",\"stringToUTF16\"]")
        end
