...
on_load(function(target)
    ...
    if goroot and goos and goarch then
        local goroot_local = path.join(config.buildir(), ".goenv",
                                       path.filename(goroot))
        local gopkgdir = path.join(os.isdir(goroot_local) and goroot_local or
        ...
        if not os.isdir(gopkgdir) or os.emptydir(gopkgdir) then
            ...
            local confirm = utils.confirm({
                default = true,
                description = ("we need to build go for %s_%s only once first!"):format(
                    goos, goarch)
            })
            if confirm then
                local build_task = function()
                    ...
                    --
                    os.vrunv(path.join(gosrcdir, "make.bash"), {"--no-clean"}, {
                        envs = {
                            GOOS = goos,
                            GOARCH = goarch,
                    ...
                    tty.erase_line_to_start().cr()
                    cprint(
                        "building go for %s_%s .. ${color.success}${text.success}",
                        goos, goarch)
                end
        ...
        if os.isdir(goroot_local) and os.isdir(gopkgdir) and
            not os.emptydir(gopkgdir) then
            config.set("gc", path.join(goroot_local, "bin", "go"),
                       {readonly = true, force = true})
            config.set("gcld", path.join(goroot_local, "bin", "go"),
                       {readonly = true, force = true})
            config.set("gcar", path.join(goroot_local, "bin", "go"),
                       {readonly = true, force = true})
        end
    end
end)
