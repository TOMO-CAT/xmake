                                                      ...
                                                      semver_range_t const* range, semvers_t* matches)
{
    ...
    for (i = lua_objlen(lua, fromidx); i > 0; --i)
    {
        ...
        if (source_str && semver_tryn(semver, source_str, xu_strlen(source_str)) == 0)
        {
                                                            ...
                                                            semvers_t* matches)
{
    ...
    for (i = lua_objlen(lua, fromidx); i > 0; --i)
    {
        ...
        xu_char_t const* source_str = luaL_checkstring(lua, -1);
        if (source_str && semver_tryn(semver, source_str, xu_strlen(source_str)) == 0) semvers_ppush(matches, *semver);
...
xu_int_t xm_semver_select(lua_State* lua)
{
    ...
    do
    {
        ...
        xu_size_t range_len = xu_strlen(range_str);
