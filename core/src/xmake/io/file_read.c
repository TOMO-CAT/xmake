...
static tb_int_t xm_io_file_buffer_pushline(tb_buffer_ref_t buf, xm_io_file_t* file, tb_char_t const* continuation, xm_bool_t keep_crlf)
{
    // check
    ...
    do
    {
        ...
        else if (size > 1)
        {
            ...
            // has continuation?
            xm_bool_t has_conline = conlen && size >= conlen + 1 && tb_strncmp(continuation, (tb_char_t const*)(data + size - conlen - 1), conlen) == 0;
...
static tb_int_t xm_io_file_read_all(lua_State* lua, xm_io_file_t* file, tb_char_t const* continuation)
{
    ...
    // read all
    xm_bool_t has_content = xm_false;
    while (1)
    {
        switch (xm_io_file_buffer_pushline(&buf, file, continuation, xm_true))
        {
        case PL_EOF:
...
static tb_int_t xm_io_file_read_line(lua_State* lua, xm_io_file_t* file, tb_char_t const* continuation, xm_bool_t keep_crlf)
{
    // check
    tb_assert(lua && file && continuation && xm_io_file_is_file(file) && file->u.file_ref);
...
static xm_size_t xm_io_file_std_buffer_pushline(tb_buffer_ref_t buf, xm_io_file_t* file, tb_char_t const* continuation, xm_bool_t keep_crlf)
{
    ...
    else if (buflen > 1)
    {
        ...
        // has continuation?
        xm_bool_t has_conline = conlen && buflen >= conlen + 1 && tb_strncmp(continuation, (strbuf + buflen - conlen - 1), conlen) == 0;
...
static tb_int_t xm_io_file_std_read_line(lua_State* lua, xm_io_file_t* file, tb_char_t const* continuation, xm_bool_t keep_crlf)
{
    // check
...
static tb_int_t xm_io_file_std_read_all(lua_State* lua, xm_io_file_t* file, tb_char_t const* continuation)
{
    ...
    while (1)
    {
        switch (xm_io_file_std_buffer_pushline(&buf, file, continuation, xm_true))
        {
        case PL_EOF:
            if (!has_content) lua_pushliteral(lua, "");
            else lua_pushlstring(lua, (tb_char_t const*)tb_buffer_data(&buf), tb_buffer_size(&buf));
...
tb_int_t xm_io_file_read(lua_State* lua)
{
    ...
    if (xm_io_file_is_file(file))
    {
        ...
        case 'L': return xm_io_file_read_line(lua, file, continuation, xm_true);
        case 'n': xm_io_return_error(lua, "read number is not implemented");
        case 'l': return xm_io_file_read_line(lua, file, continuation, xm_false);
        default:
    ...
    else
    {
        ...
        case 'L': return xm_io_file_std_read_line(lua, file, continuation, xm_true);
        case 'n': return xm_io_file_std_read_num(lua, file, continuation);
        case 'l': return xm_io_file_std_read_line(lua, file, continuation, xm_false);
        default:
            xm_io_return_error(lua, "unknonwn read mode");
            return 0;
        }
