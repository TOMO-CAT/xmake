...
static xu_void_t xu_fwatcher_item_callback(ConstFSEventStreamRef stream, xu_pointer_t priv,
    size_t events_count, xu_pointer_t event_paths, const FSEventStreamEventFlags event_flags[], FSEventStreamEventId const* event_id)
{
    // check
    ...
    for (i = 0; i < events_count; ++i)
    {
        ...
        if (filepath) xu_strlcpy(event.filepath, filepath, XU_PATH_MAXN);
        ...
        else
        {
            ...
            if (p && p - event.filepath == xu_strlen(watchitem->watchdir))
                matched = xu_true;
        }
        ...
        if (event.event)
        {
            ...
            if (!xu_queue_full(fwatcher->events_queue))
                xu_queue_put(fwatcher->events_queue, &event);
...
static xu_bool_t xu_fwatcher_item_init(xu_fwatcher_t* fwatcher, xu_char_t const* watchdir, xu_fwatcher_item_t* watchitem)
{
    // check
...
#endif
    watchitem->stream = FSEventStreamCreate(xu_null, xu_fwatcher_item_callback, context,
...
xu_bool_t xu_fwatcher_add(xu_fwatcher_ref_t self, xu_char_t const* watchdir, xu_bool_t recursion)
{
    ...
    return xu_hash_map_insert(fwatcher->watchitems, watchdir_real, &watchitem) != xu_iterator_tail(fwatcher->watchitems);
}
