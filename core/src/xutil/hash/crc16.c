...
xu_uint16_t const g_crc16_table[] = {
...
xu_uint16_t const g_crc16_ccitt_table[] = {
...
xu_uint16_t xu_crc16_make(xu_byte_t const* data, xu_size_t size, xu_uint16_t seed)
{
    ...
    // init value
    xu_uint32_t crc = seed;

    // done
    xu_byte_t const*   ie = data + size;
    xu_uint16_t const* pt = (xu_uint16_t const*)g_crc16_table;
    while (data < ie)
        crc = pt[((xu_uint8_t)crc) ^ *data++] ^ (crc >> 8);

    // ok?
    return (xu_uint16_t)crc;
}
xu_uint16_t xu_crc16_make_from_cstr(xu_char_t const* cstr, xu_uint16_t seed)
{
    ...
    // make it
    return xu_crc16_make((xu_byte_t const*)cstr, xu_strlen(cstr) + 1, seed);
}
xu_uint16_t xu_crc16_ccitt_make(xu_byte_t const* data, xu_size_t size, xu_uint16_t seed)
{
    ...
    // init value
    xu_uint16_t crc = seed;

    // done
    xu_byte_t const*   ie = data + size;
    xu_uint16_t const* pt = (xu_uint16_t const*)g_crc16_ccitt_table;
    while (data < ie)
        crc = pt[((xu_uint8_t)crc) ^ *data++] ^ (crc >> 8);
...
xu_uint16_t xu_crc16_ccitt_make_from_cstr(xu_char_t const* cstr, xu_uint16_t seed)
{
    ...
    // make it
    return xu_crc16_ccitt_make((xu_byte_t const*)cstr, xu_strlen(cstr) + 1, seed);
}
