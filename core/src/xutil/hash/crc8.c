#include "xutil/hash/crc8.h"
#include "xutil/libc/string/string.h"
#include "xutil/macros/assert.h"

/* *******************************************************
 * globals
 */

// the crc8(ANSI) table
xu_uint8_t const g_crc8_table[] = {
    };

/* *******************************************************
 * implementation
 */

xu_uint8_t xu_crc8_make(xu_byte_t const* data, xu_size_t size, xu_uint8_t seed)
{
    // check
    xu_assert_and_check_return_val(data, 0);

    // init value
    xu_uint32_t crc = seed;

    // done
    xu_byte_t const*  ie = data + size;
    xu_uint8_t const* pt = (xu_uint8_t const*)g_crc8_table;
    while (data < ie)
        crc = pt[((xu_uint8_t)crc) ^ *data++] ^ (crc >> 8);

    // ok?
    return (xu_uint8_t)crc;
}
xu_uint8_t xu_crc8_make_from_cstr(xu_char_t const* cstr, xu_uint8_t seed)
{
    // check
    xu_assert_and_check_return_val(cstr, 0);

    // make it
    return xu_crc8_make((xu_byte_t const*)cstr, xu_strlen(cstr) + 1, seed);
}
