...
if [ -d "${SDK_DIR}" ]; then
    ...
    if [ -f "${SDK_ZIP_FILE}" ]; then
        ...
        wget "https://github.com/TOMO-CAT/zxmake-mirror/releases/download/20260510.1/arm-gnu-toolchain-13.3.rel1-x86_64-arm-none-linux-gnueabihf.tar.xz" -O ${SDK_ZIP_FILE}
    fi
    tar -xf ${SDK_ZIP_FILE}
fi
